...
This document defines a strategy to securely assign a candidate device (Pledge) to an Owner using a digital artifact
...
Capitalized terms such as Pledge, Owner, Domain, Onboarding and MASA are defined in {{terminology}}.

{{motivation}} explains why Onboarding protocols need such an artifact and why alternative approaches are undesirable.
...
It is encoded using the rules defined in {{!RFC7951}} or {{RFC9254}}, and is signed using (by default)
a CMS structure {{RFC5652}}.

This document obsoletes {{RFC8366}} and updates {{BRSKI}}.
It adds the Attributes needed by the variants of {{BRSKI}} that were developed since {{RFC8366}} was published into the
Voucher YANG module, it includes and updates the Voucher Request YANG module that was defined in {{BRSKI}},
and it adds a mechanism for future extensions that avoids further revisions of the YANG modules.
The motivation for these changes, and the detailed list of changes, are given in {{changes-since-rfc8366}}.

The remainder of this document is organized as follows.
{{terminology}} defines the terms that are used throughout.
{{survey}} surveys the information that a Voucher can carry, and the combinations of that information
that constitute different types of Voucher.
{{changes-since-rfc8366}} and {{updates-rfc8995}} detail the changes relative to {{RFC8366}} and {{BRSKI}}.
{{signature-mechanisms}} describes the default signature mechanism (CMS) for Voucher Artifacts.
{{voucher}} and {{voucher-request}} give the normative definitions of the two artifacts,
including their tree diagrams, YANG modules and SID values.
{{design-con}} and {{sec-con}} discuss the design considerations and the security considerations, respectively.

# Motivation {#motivation}

This section is informative.
It explains the problems that Voucher-based Onboarding solves, and why the alternative approaches are undesirable.

## Onboarding without a Voucher

When a device is sold to a new owner, the device normally has no way to verify the identity of that new owner,
nor to limit its own configurability to that new owner.
Instead, such devices (Pledges) employ one or more of the following problematic mechanisms:

1. They permit configuration without any security or password, or they
   require a well-known "default" password.

2. They limit configuration to mechanisms that require some degree of
   physical control over the device, such as permitting configuration only
   across a serial "console" port, or requiring pairing via Bluetooth.

3. They are delivered with some form of "bearer token", such as a unique
...
All these mechanisms expose various security or operational problems,
making them insufficient for the typical professional deployment of equipment such as is required in industrial,
enterprise, defense and other markets for networking equipment or other IoT equipment.
...
that discovers and invades unconfigured devices via their default passwords, and so on.

In one evaluation, professional video surveillance cameras deployed at scale cost $500 on average,
...
harmful software cost less than $500.

## Staging as a remedy?

To overcome the provisioning limitations of pre-voucher protocol solutions,
...
at least to the extent that subsequent attacks or unintentional misconfiguration by a non-owner are no longer possible.
...
system to be solved -- which is exactly what protocols using Vouchers aim to achieve.

## Onboarding with a Voucher

The Voucher is a digital artifact (a digital data structure) that is created and cryptographically signed by an agent of
the Pledge's manufacturer, called the Manufacturer Authorized Signing Authority (MASA).

The Voucher's most important element is a trust anchor that identifies the intended Owner of the Pledge.
This trust anchor is typically a certificate (the '`pinned-domain-cert`' Attribute),
...
and the act of communicating it is known as pinning the trust anchor.

The rationale for this approach is that the only trust anchors an unconfigured Pledge can be expected to carry in its
...
How the MASA determines who the Owner of a Pledge is, is left to the Onboarding protocol mechanisms using the Voucher.

Once the Pledge has received such a Voucher via some protocol utilizing a Voucher,
...
{{?RFC8994}} explains how the pinned trust anchor is used to form an overlay management Autonomic Control Plane (ACP)
...
although authorization is best left to mechanisms such as {{?RFC9200}}.

The Voucher itself does not specify the procedures by which it is communicated.
That is the responsibility of the Onboarding protocols that use Voucher Artifacts.

In "Secure Zero Touch Provisioning" {{SZTP}}, protocol mechanisms are defined through which a Pledge can automatically
...
identifying the Pledge's Owner.

In "Bootstrapping Remote Secure Key Infrastructure" {{BRSKI}} and its derivative protocols,
...
as well as a so-called LDevID certificate (operational certificate) from that Domain.

SZTP and BRSKI are only two examples of existing protocol families that use Vouchers.
They follow different design philosophies and thereby illustrate why the Voucher is not tied to,
and not specified solely within, a single protocol family.

## What the Voucher does and does not provide

Use of the Voucher as specified in this document does not guarantee interoperability between an arbitrary Pledge and an
arbitrary configuration/provisioning/enrollment/Onboarding server; that is instead the responsibility
of the protocols using Vouchers.

What the use of a Voucher-based Onboarding protocol does enable is the ability to enroll and provision Pledges without
...
which is a common problem in multi-tenant and multi-dwelling environments.

# Terminology {#terminology}

{::boilerplate bcp14-tagged}

This document uses and defines the following terms, which are also used in related documents.
The terms are listed in alphabetical order.

Artifact:
: See "Voucher Artifact".
...
Pinning:
: The act of securely communicating a trust anchor to a Pledge by conveying it in a Voucher.
  The trust anchor that is communicated this way is said to be pinned.
...
Trust Anchor:
: A public key, or a certificate that contains a public key, that a Pledge is configured to trust as an authoritative
  source when it validates the credentials that other entities present to it; see also {{?RFC6024}}.
  The Voucher conveys the trust anchor of the Owner's Domain to the Pledge.
...
Zero-touch:
: Describes an Onboarding process that requires no manual configuration of the Pledge at its deployment location,
  beyond the physical installation of the Pledge.
...
Voucher lifetimes vary between Onboarding protocols.
...
# Changes since RFC8366 {#changes-since-rfc8366}

This document obsoletes {{RFC8366}}.

## Motivation for revising RFC8366 {#revising-rfc8366}

{{RFC8366}} originally defined the Voucher as the only Voucher Artifact,
leaving the Voucher Request that is used in BRSKI to be defined in {{BRSKI}}.
This document includes both Voucher and Voucher Request thereby obsoleting {{RFC8366}}, and Updating {{BRSKI}}.

A number of variations of {{BRSKI}} have been developed since the publication of {{RFC8366}},
...
(or CBOR) maps, and adding new keys seemed easy.

However, the use of YANG for the information model does not make it as trivial as was thought.
...
The process that led to this conclusion is described in {{history}}.

This document introduces a mechanism to support future extensions without requiring the YANG module to be revised.
This includes both a new IETF standard mechanism for extensions modeled after the mechanism present in {{?RFC8520}},
as well as a facility for manufacturer proprietary extensions.

## History of New Protocols Since RFC8366 {#history}
...
# Updates to RFC8995 {#updates-rfc8995}
...
# Signature mechanisms {#signature-mechanisms}
...
Further examples of CMS-signed Vouchers are given in {{examples}}.

## YANG Module "ietf-voucher" {#voucher-yang-module}
...
The processes which were anticipated to be useful (the YANG "augment" mechanism), turned out not to be, see {{revising-rfc8366}}.
...
An example of a CMS-signed Voucher Request is given in {{examples}}.

## YANG Module "ietf-voucher-request" {#voucher-request-yang-module}
